...
import {
  DEFAULT_MAEST_LABELS_URL,
  fetchValidGraphManifest,
  findMissingWeightShard,
  getLabelsUrlFromGraphManifest,
  normalizeGraphCandidate,
} from "./modelAssetProbe.js";
...
const GENRE_BPM_RANGES = {
  ...
  edm: { min: 110, max: 178, weight: 0.1 },
  hiphop: { min: 65, max: 115, weight: 0.14 },
  pop: { min: 85, max: 140, weight: 0.08 },
  eighties: { min: 100, max: 145, weight: 0.1 },
...
let smoothedGenreScores = Object.fromEntries(
  ALL_GENRES.map((genre) => [genre, 1 / ALL_GENRES.length]),
);
...
export async function startGenreDetector(audioContext, cb) {
  ...
  smoothedGenreScores = Object.fromEntries(
    ALL_GENRES.map((genre) => [genre, 1 / ALL_GENRES.length]),
  );
...
async function runAnalysis() {
  ...
  try {
    const windowSamples = normalizeMonoSamples(
      audioBuffer.slice(audioBuffer.length - minimumWindowSamples),
    );
...
function selectGenre(scores) {
  ...
  const weightedSorted = Object.entries(weightedScores).sort(
    (a, b) => b[1] - a[1],
  );
  ...
  if (rawTopScore < confidenceThreshold || rawDelta < marginThreshold) {
    ...
    const closeRaw =
      Math.abs(rawTopScore - weightedAltRaw) <= marginThreshold * 1.5;
...
function smoothScores(scores) {
  ...
  for (const genre of ALL_GENRES) {
    ...
    next[genre] =
      prev * (1 - SCORE_SMOOTHING_ALPHA) + curr * SCORE_SMOOTHING_ALPHA;
  }
...
async function loadEssentia() {
  ...
  essentiaLoadPromise = (async () => {
    try {
      ...
      const essentiaUrl = new URL(
        "/models/essentia-wasm.es.js",
        window.location.origin,
      ).href;
      const importedModule = await import(/* @vite-ignore */ essentiaUrl).catch(
        () => null,
      );
      if (!importedModule) {
        detectorStatus = {
          ...
          detail:
            "Essentia runtime files were not found in /public/models/. Run npm run setup:models to copy the browser runtime bundle.",
        };
        console.warn(
          "[GenreDetector] Essentia.js not found in /public/models/ — using spectral heuristic fallback. Run `npm run setup:models` to install the runtime files.",
        );
        return;
      }
      ...
      if (!wasmModule) {
        detectorStatus = {
          ...
          detail:
            "Essentia runtime files were found, but the browser module did not expose a compatible WASM API.",
        };
        console.warn(
          "[GenreDetector] Essentia.js module loaded, but no compatible WASM export was found — using spectral heuristic fallback",
        );
        return;
      }
      ...
      availableMaestLabelsUrl =
        availableMaestGraph?.labelsUrl || DEFAULT_MAEST_LABELS_URL;
      maestLabels = await loadMaestLabels(availableMaestLabelsUrl);
      modelLoaded = Boolean(
        essentiaModule && maestLabels?.length && availableMaestGraphFilename,
      );
      ...
      if (!maestLabels?.length) {
        ...
        console.warn(
          "[GenreDetector] Essentia loaded but MAEST labels are missing or incompatible with prediction output format; enabling spectral heuristic fallback.",
        );
        return;
      }
      ...
      if (!availableMaestGraphFilename) {
        detectorStatus = {
          ...
          detail:
            "Essentia loaded, but no TensorFlow.js MAEST graph model was found. Add public/models/maest-30s-pw/model.json plus every referenced group*.bin shard. A standalone .onnx file is not enough.",
        };
        console.warn(
          "[GenreDetector] Essentia loaded, but no compatible MAEST TensorFlow graph model was found in /public/models/. Falling back to spectral heuristic. Expected a TensorFlow.js graph model such as /models/maest-30s-pw/model.json. A standalone .onnx file is not sufficient.",
        );
        return;
      }
      ...
      console.log(
        "[GenreDetector] Essentia.js loaded successfully",
        `(labels: ${maestLabels.length}, graph: ${availableMaestGraphFilename})`,
      );
    } catch (err) {
      detectorStatus = {
        mode: "heuristic",
...
async function runEssentiaModel(samples) {
  ...
  try {
    ...
    const resampled = resampleWithFilter(
      samples,
      INPUT_SAMPLE_RATE,
      MODEL_SAMPLE_RATE,
    );
    ...
    if (!predictions.length) {
      console.warn(
        "[GenreDetector] MAEST prediction parsing produced no label/score pairs; falling back to spectral heuristic.",
      );
      return spectralHeuristic(samples);
    }
    ...
    console.warn(
      `[GenreDetector] Model inference failed (${errorMessage}). Disabling Essentia inference for the rest of this session and falling back to the spectral heuristic.`,
    );
    return spectralHeuristic(samples);
  } finally {
...
async function predictMaest(essentia, features) {
  ...
  try {
    const predictions = await essentia.TensorflowPredict2D(features, {
      graphFilename: availableMaestGraphFilename,
    });
    ...
    throw new Error(
      `Graph ${availableMaestGraphFilename} failed: ${formatErrorMessage(err)}`,
    );
  }
}
  ...
  for (const candidate of [
    ...
    try {
      ...
      if (missingWeightShard) {
        console.warn(
          `[GenreDetector] Ignoring MAEST graph ${normalizedCandidate} because a referenced weight shard could not be loaded: ${missingWeightShard}`,
        );
        continue;
      }
      ...
      console.warn(
        `[GenreDetector] Ignoring MAEST graph ${normalizedCandidate} because it is not a valid TensorFlow.js graph model: ${formatErrorMessage(err)}`,
      );
    }
  }
...
function parsePredictionOutput(predictions) {
  ...
  if (Array.isArray(predictions) && Array.isArray(predictions[0])) {
    ...
    if (typeof predictions[0][0] === "number" && maestLabels?.length) {
      ...
      return flatScores.map((score, index) => [
        maestLabels[index] || `label_${index}`,
        score,
      ]);
    }
  }
  ...
  if (
    ...
    const withLabelScore = predictions
      ...
      .filter(
        ([label, score]) => typeof label === "string" && Number.isFinite(score),
      );
  ...
  if (!maestLabels?.length) {
    console.warn(
      "[GenreDetector] Unable to parse MAEST numeric predictions because labels are unavailable; using fallback path.",
    );
  }
...
function normalizeScoreMap(scoreMap) {
  const normalized = Object.fromEntries(
    ALL_GENRES.map((genre) => [genre, scoreMap[genre] || 0]),
  );
  const total =
    Object.values(normalized).reduce((sum, score) => sum + score, 0) || 1;
...
function lowPassFilter(samples, sampleRate, cutoffHz) {
  ...
  for (let i = 0; i < filterLength; i++) {
    ...
    const window =
      0.42 -
      0.5 * Math.cos((2 * Math.PI * i) / (filterLength - 1)) +
