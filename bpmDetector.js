...
const MEDIAN_HISTORY_SIZE = 11
const BPM_MEDIAN_WEIGHT = 0.6  // Heavier median blend suppresses short-lived jitter
const LOCKED_OUTPUT_MAX_STEP = 0.85
const UNLOCKED_OUTPUT_MAX_STEP = 1.8
const LOCKED_OUTPUT_ALPHA = 0.35
const UNLOCKED_OUTPUT_ALPHA = 0.55
const TEMPO_ANCHOR_RELEASE_FRAMES = 2000
const OSC_LOCK_HISTORY_FRAMES = 30
const OSC_LOCK_ACQUIRE_FRAMES = 30
const OSC_LOCK_HOLD_BAND = 2
const OSC_LOCKED_MAX_STEP = 0.35
const OSC_LOCKED_ALPHA = 0.22
...
let displayedBpm = 120
let displayedBpmHistory = []
let tempoShiftCounter = 0
let tempoAnchorBpm = null
let oscBpmLock = null
let tempoLockedFrames = 0
let oscBpmLockStableFrames = 0
let oscBpmLockMismatchFrames = 0
...
export async function startBPMDetector(audioContext, sourceNode, cb) {
  ...
  oscBpmLock = null
  tempoLockedFrames = 0
  oscBpmLockStableFrames = 0
  oscBpmLockMismatchFrames = 0
  smoothedEnergy = 0
  smoothedCentroid = 0
  smoothedFlux = 0
  smoothedLowBand = 0
...
function handleFrame(features) {
  ...
  const tempoReference = displayedBpmHistory.length >= 30
    ? getMedian(displayedBpmHistory)
    : smoothedBpm
  ...
  if (isOnset) {
    ...
    const onsetBpm = estimateOnsetBPM(onsetHistory, tempoReference)
    if (onsetBpm) {
      const beatLockStrength = estimateBeatLockStrength(onsetHistory, tempoReference)
      const beatPresent = (now - lastOnsetTime) <= BEAT_ACTIVE_WINDOW_MS
      const delta = Math.abs(onsetBpm - smoothedBpm)

      let onsetWeight = 0.12
      if (beatPresent && beatLockStrength > 0.7) {
        onsetWeight = delta <= BPM_LOCK_TOLERANCE ? 0.08 : 0.015
  ...
  if (!isSilent && (now - lastTempoEstimate) >= TEMPO_ESTIMATE_INTERVAL_MS) {
    ...
    const envelopeBpm = estimateEnvelopeBPM(envelopeHistory, tempoReference)
    if (envelopeBpm) {
      const beatLockStrength = estimateBeatLockStrength(onsetHistory, tempoReference)
      const beatPresent = (now - lastOnsetTime) <= BEAT_ACTIVE_WINDOW_MS
      const delta = Math.abs(envelopeBpm - smoothedBpm)

      let envelopeWeight = 0.14
      if (beatPresent && beatLockStrength > 0.7) {
        envelopeWeight = delta <= BPM_LOCK_TOLERANCE ? 0.05 : 0.01
      } else if (beatLockStrength > 0.45 && delta > BPM_LOCK_HARD_TOLERANCE) {
        envelopeWeight = 0.03
      }
  ...
  smoothedBpm = normalizeBpmToReference(smoothedBpm, tempoReference)
  ...
  const blendedBpm = smoothedBpm * (1 - BPM_MEDIAN_WEIGHT) + medianBpm * BPM_MEDIAN_WEIGHT

  // Final output smoother: limit per-frame tempo jumps while locked on a beat.
  const beatLockStrength = estimateBeatLockStrength(onsetHistory, tempoReference)
  const tempoLocked = !isSilent && beatLockStrength > 0.62 && onsetHistory.length >= 8
  if (tempoLocked) {
    tempoLockedFrames = Math.min(2000, tempoLockedFrames + 1)
  } else {
    tempoLockedFrames = Math.max(0, tempoLockedFrames - 2)
  }
  if (tempoAnchorBpm == null && displayedBpmHistory.length >= 40) {
    tempoAnchorBpm = getMedian(displayedBpmHistory)
  }

  const anchorReady = Number.isFinite(tempoAnchorBpm)
  const anchorBpm = anchorReady ? tempoAnchorBpm : displayedBpm
  const guardBand = tempoLocked ? 5 : 7
  const outsideGuard = Math.abs(blendedBpm - anchorBpm) > guardBand
  if (outsideGuard) {
    tempoShiftCounter++
  } else {
    tempoShiftCounter = Math.max(0, tempoShiftCounter - 3)
  }

  // Keep short-term outliers from dragging tempo off a stable lock.
  // If a new tempo persists for multiple minutes, disable the guard and allow migration.
  const guardEnabled = !isSilent && anchorReady && tempoShiftCounter < TEMPO_ANCHOR_RELEASE_FRAMES
  const guardedTarget = guardEnabled
    ? clamp(blendedBpm, anchorBpm - guardBand, anchorBpm + guardBand)
    : blendedBpm

  if (
    !Number.isFinite(oscBpmLock)
    && tempoLocked
    && tempoLockedFrames >= OSC_LOCK_ACQUIRE_FRAMES
    && displayedBpmHistory.length >= OSC_LOCK_HISTORY_FRAMES
  ) {
    const lockCandidate = getMedian(displayedBpmHistory.slice(-OSC_LOCK_HISTORY_FRAMES))
    oscBpmLock = lockCandidate
    oscBpmLockStableFrames = Math.min(1200, oscBpmLockStableFrames + 1)
  } else if (!tempoLocked) {
    oscBpmLockStableFrames = Math.max(0, oscBpmLockStableFrames - 1)
  }

  let oscStabilizedTarget = guardedTarget
  const lockActive = Number.isFinite(oscBpmLock)
  if (lockActive) {
    const mismatch = Math.abs(guardedTarget - oscBpmLock)
    if (mismatch > OSC_LOCK_HOLD_BAND) {
      oscBpmLockMismatchFrames += tempoLocked ? 1 : 4
    } else {
      oscBpmLockMismatchFrames = Math.max(0, oscBpmLockMismatchFrames - 6)
    }

    const lockWeight = Math.min(0.75, 0.45 + (oscBpmLockStableFrames / 500))
    oscStabilizedTarget = guardedTarget * (1 - lockWeight) + oscBpmLock * lockWeight
    oscStabilizedTarget = clamp(
      oscStabilizedTarget,
      oscBpmLock - OSC_LOCK_HOLD_BAND,
      oscBpmLock + OSC_LOCK_HOLD_BAND,
    )
  ...
  const maxStep = lockActive
    ? OSC_LOCKED_MAX_STEP
    : (tempoLocked ? LOCKED_OUTPUT_MAX_STEP : UNLOCKED_OUTPUT_MAX_STEP)
  const alpha = lockActive
    ? OSC_LOCKED_ALPHA
    : (tempoLocked ? LOCKED_OUTPUT_ALPHA : UNLOCKED_OUTPUT_ALPHA)
  const boundedTarget = displayedBpm + clamp(oscStabilizedTarget - displayedBpm, -maxStep, maxStep)
  displayedBpm = displayedBpm * (1 - alpha) + boundedTarget * alpha
  displayedBpmHistory.push(displayedBpm)
  if (displayedBpmHistory.length > 180) {
    displayedBpmHistory.shift()
  }

  if (!guardEnabled && anchorReady) {
    tempoAnchorBpm = tempoAnchorBpm * 0.9 + blendedBpm * 0.1
  }

  const finalBpm = Math.round(displayedBpm)
...
function estimateOnsetBPM(timestamps, referenceBpm) {
  const intervalBpm = estimateTempoFromPeakIntervals(timestamps, referenceBpm, {
    minBpm: MIN_BPM,
    maxBpm: MAX_BPM,
  ...
  return normalizeBpmToReference(disambiguated, referenceBpm, MIN_BPM, MAX_BPM)
}
