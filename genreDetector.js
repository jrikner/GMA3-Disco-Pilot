...
async function runGraphModel(model, inputs, outputName = null) {
  if (typeof model?.execute === 'function') {
    try {
      ...
      // Fall back to executeAsync for graphs with control-flow nodes.
    }
  }

  if (typeof model?.executeAsync === 'function') {
    return outputName ? await model.executeAsync(inputs, outputName) : await model.executeAsync(inputs)
  }

  throw new Error('MAEST graph model does not expose execute or executeAsync.')
}
